...
### 4.1 图片存入 .mdx
- [ ] 编辑器中插入图片时，通过 IPC 发送图片数据到主进程
- [ ] 主进程将图片写入 .mdx 临时解压目录的 assets/images/
- [ ] 更新 mdx.json 中的 assets 列表
- [ ] 编辑器中图片 src 替换为 assets/images/xxx.png 相对路径
- [ ] 保存时将临时目录重新打包为 .mdx

### 4.2 图片自动重命名与去重
- [ ] 计算图片文件内容 hash（SHA256 前 12 位）
- [ ] 以 hash 重命名图片文件（避免中文/特殊字符文件名问题）
- [ ] 插入前检查 hash 是否已存在，存在则复用不重复存储
- [ ] 更新所有引用该图片的路径

### 4.3 图片压缩
- [ ] 安装 sharp 依赖（主进程端压缩）
- [ ] 实现图片压缩函数（支持 JPEG/PNG/WebP）
- [ ] 压缩设置 UI：质量滑块（1-100）
- [ ] 压缩设置 UI：最大宽度/高度限制
- [ ] 压缩设置 UI：自动压缩开关（插入时自动压缩）
- [ ] 压缩前后大小对比显示

### 4.4 资源管理器面板
- [ ] 创建 AssetManager.vue 组件（模态框）
- [ ] 列出当前 .mdx 中所有资源文件（缩略图 + 文件名 + 大小）
- [ ] 点击资源插入到编辑器
- [ ] 删除资源（同时移除编辑器中的引用）
- [ ] 替换资源（保留引用路径，替换文件内容）
- [ ] 资源排序（按名称/大小/类型）
...
### 4.5 图片引用路径管理
- [ ] 删除图片时自动清理 Markdown 中的引用
- [ ] 重命名图片时自动更新引用路径
- [ ] 检测孤立资源（未被引用的图片）并提示清理
- [ ] 引用路径验证：打开文件时检查引用的资源是否存在

### 4.6 附件管理
- [ ] 扩展 assets 目录结构（attachments/ 子目录）
- [ ] 支持插入任意文件附件
- [ ] 附件在编辑器中显示为卡片（文件名 + 大小 + 下载按钮）
- [ ] 附件点击可打开/保存到本地
...
### 5.4 撤销/重做
- [ ] WYSIWYG 模式：ProseMirror 内置 undo/redo
- [ ] 源码模式：CodeMirror 内置 undo/redo
...
|------|--------|-----------|
| 阶段 1：项目基础搭建 | 26 | ~26h |
| 阶段 2：.mdx 文件格式 | 31 | ~35h |
| 阶段 3A：源码编辑器 | 25 | ~30h |
| 阶段 3B：WYSIWYG 编辑器 | 35 | ~45h |
| 阶段 4：图片与资源管理 | 25 | ~30h |
| 阶段 5：导出与高级功能 | 33 | ~39h |
| 阶段 6：打磨与发布 | 30 | ~36h |
| **合计** | **205** | **~241h** |
