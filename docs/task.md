...
> 以下完成状态以当前自动化测试为准。AI 助手是按需打开、可关闭的独立标签，不改变文件标签的保存与恢复语义。

- [x] Provider 配置、安全保存 API Key，并分别探测文本生成与 Tool Calling 能力
- [x] AI 标签启动时隐藏，可从界面打开、关闭和重新打开；关闭空闲标签保留内存会话
- [x] 流式对话、停止生成、工具状态和单会话单运行限制
- [x] 白名单工具：读取当前文档/选区、读取用户消息中明确给出的网页或本地文件，以及审批后替换、插入或新建文档
- [x] 模型根据自然语言选择来源读取工具；Main 从用户消息独立提取并精确校验 HTTPS URL 或带引号的 Windows 绝对路径
- [x] `read_web_url` 与 `read_local_file` 均逐次审批且只执行一次；网页和本地文件读取均在 Main 完成，不提供任意网络浏览或文件系统访问
- [x] 来源读取无状态提取并执行文件类型、网络、响应、文本、分块、超时和重定向限制；本地文件审批预览只显示元数据
- [x] 写工具由运行时策略强制逐次审批，Prompt 和工具参数不能关闭或跳过审批
- [x] 审批后仍校验目标标签、文档版本和内容摘要，冲突时不覆盖用户的新编辑
- [x] 来源网络边界、大小/超时/步骤限制、敏感日志与 API Key 隔离
- [x] 明确排除 OCR、持久化知识库、模型自由浏览网络、模型指定本地路径及删除/移动/重命名文件和命令执行工具
...
| 阶段 5：导出与高级功能 | ~24 | ~11 | 🔄 部分完成（PDF/自动保存/撤销重做/崩溃恢复已完成） |
| 阶段 6：打磨与发布 | ~14 | ~20 | 🔄 部分完成（打包 + CI + 自动更新已完成） |
